...
const mockInsights = {
  healthScore: {
    ...
    trend: "+0.5",
    analysis:
      "Account is healthy and stable. Champion Mike Johnson is highly engaged, product usage is strong (82%), and support tickets are minimal. However, feature adoption lags at 5/8 features - this represents untapped value and potential churn risk if competitors offer simpler alternatives.",
    actions: [
      {
  ...
  productUsage: {
    ...
    trend: "+12%",
    analysis:
      "Excellent adoption! 24 of 28 users actively log in daily. Power users Sarah Chen and Mike Johnson drive 60% of usage. Risk: Over-reliance on 2 users. If either leaves, usage could drop significantly. 4 inactive users are in Finance department - they may not see value yet.",
    actions: [
      {
  ...
  risks: [
    {
      ...
      description:
        "Only 5 of 8 features adopted. Missing features: Advanced Analytics, Workflow Automation, API Integration. These are key differentiators vs competitors and represent $40K of their $240K contract value. Low adoption makes renewal vulnerable to competitive pressure on price.",
      mitigation: [
  ...
  opportunities: [
    {
      ...
      confidence: 85,
      description:
        'Mike Johnson mentioned in last call: "We\'re hiring 15 more operations analysts and will need additional seats." + Finance team (4 inactive users) needs budget module. Total expansion: 15 seats ($36K) + Premium Analytics ($24K) = $60K ARR. Timing is perfect - annual budget planning happening now.',
      actions: [
        {
          phase: "This Week",
...
interface CSAccountDetailViewProps {
  accountId: string
}

export function CSAccountDetailView({ accountId }: CSAccountDetailViewProps) {
  const account = mockAccount // In real app, fetch by id
  ...
  return (
    <div className="space-y-8 max-w-7xl mx-auto px-6 py-8">
      ...
      {/* Page Header */}
      <div className="bg-gradient-to-br from-primary via-primary to-primary/90 rounded-xl p-6 text-primary-foreground shadow-lg shadow-primary/20">
          <Link
            href="/cs/accounts"
          ...
          <div className="flex justify-between items-start">
            <div>
              <h1 className="text-4xl font-bold mb-3 tracking-tight">{account.name}</h1>
              <div className="text-base opacity-90">
                {account.segment} · {formatCurrency(account.arr)} ARR · Renewal:{" "}
            ...
            <div className="flex gap-3">
              ...
              <Button variant="outline" className="bg-transparent border-background text-background hover:bg-background/20">
                Export Report
              </Button>
            </div>
          </div>
      ...
      {/* Alert Banner */}
      <div className="bg-amber-500/10 border-l-4 border-amber-500/80 rounded-xl p-6 flex justify-between items-center shadow-sm">
          <div className="flex-1">
            <div className="text-lg font-bold text-amber-900 mb-2">⚠️ Action Required: Q4 QBR Overdue</div>
            <div className="text-sm text-amber-800 leading-relaxed">
              Last QBR was 28 days ago. With renewal in {account.daysUntilRenewal} days, schedule immediately to discuss
              expansion opportunities and address feature adoption gaps.
            </div>
          </div>
          <Button className="bg-amber-500/90 text-white hover:bg-amber-500 shadow-md">Schedule Now</Button>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          ...
          <div className="col-span-2 space-y-8">
            ...
            <Card className="bg-gradient-to-br from-card to-card/95 border-l-4 border-l-primary/80">
              <CardContent className="p-6">
                ...
                <div className="bg-muted p-4 rounded-lg mb-4">
                  <div className="text-sm font-bold text-foreground mb-2">📊 What This Means:</div>
                  <div className="text-sm text-muted-foreground leading-relaxed">{mockInsights.healthScore.analysis}</div>
                </div>

                <div className="bg-primary/10 p-4 rounded-lg">
                  <div className="text-sm font-bold text-primary mb-3 flex items-center gap-2">🎯 Recommended Actions:</div>
                  {mockInsights.healthScore.actions.map((action, idx) => (
                    <div key={idx} className="text-sm text-foreground mb-3 pl-6 relative">
            ...
            <Card className="bg-gradient-to-br from-card to-card/95 border-l-4 border-l-primary/80">
              <CardContent className="p-6">
                <div className="flex justify-between items-start mb-4">
                  <div>
                    <div className="text-xs uppercase font-semibold text-muted-foreground mb-1">Product Usage Rate</div>
                    <div className="text-4xl font-bold text-foreground mb-1">{mockInsights.productUsage.value}%</div>
                    <div className="text-sm font-semibold text-green-600">
                ...
                <div className="bg-muted p-4 rounded-lg mb-4">
                  <div className="text-sm font-bold text-foreground mb-2">📊 Deep Dive:</div>
                  <div className="text-sm text-muted-foreground leading-relaxed">{mockInsights.productUsage.analysis}</div>
                </div>
            ...
            {mockInsights.risks.map((risk, idx) => (
              <Card key={idx} className="bg-gradient-to-br from-card to-card/95 border-l-4 border-l-destructive/80">
                <CardContent className="p-6">
                  ...
                  <div className="text-sm text-red-800 mb-4 leading-relaxed">{risk.description}</div>
                  <div className="bg-card p-3 rounded-md">
                    <div className="text-sm font-bold text-red-900 mb-2">Mitigation Plan (Execute by Nov 15):</div>
                    {risk.mitigation.map((step, stepIdx) => (
                      <div key={stepIdx} className="text-sm text-foreground mb-2 pl-6 relative">
            ...
            {mockInsights.opportunities.map((opp, idx) => (
              <Card key={idx} className="bg-gradient-to-br from-card to-card/95 border-l-4 border-l-primary/80">
                <CardContent className="p-6">
                  <div className="flex justify-between items-start mb-4">
                    <div>
                      <div className="text-xs uppercase font-semibold text-green-700 mb-1">💰 Expansion Opportunity Identified</div>
                      <div className="text-4xl font-bold text-green-700 mb-1">+{formatCurrency(opp.value)} ARR Potential</div>
                      <div className="text-sm font-semibold text-green-700">{opp.confidence}% Confidence</div>
                    </div>
                  </div>

                  <div className="bg-muted p-4 rounded-lg mb-4">
                    <div className="text-sm font-bold text-foreground mb-2">💡 The Opportunity:</div>
                    <div className="text-sm text-muted-foreground leading-relaxed">{opp.description}</div>
                  </div>
          ...
          <div className="space-y-8">
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-xl font-semibold">Renewal Timeline</CardTitle>
              </CardHeader>
              <CardContent className="p-6 space-y-3">
                <div className="bg-amber-50 p-4 rounded-lg">
                  <div className="text-xs uppercase font-semibold text-muted-foreground mb-1">Days Until Renewal</div>
                  <div className="text-3xl font-bold text-amber-600 mb-2">{account.daysUntilRenewal} Days</div>
                  ...
                  <div className="text-xs text-primary font-semibold mt-2">→ Critical: Schedule QBR before Nov 10</div>
                </div>
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-xl font-semibold">Stakeholder Map</CardTitle>
              </CardHeader>
              <CardContent className="p-6 space-y-3">
                {mockInsights.stakeholders.map((stakeholder, idx) => (
                  ...
                  >
                    ...
                    <div className="text-xs text-foreground leading-relaxed space-y-1">
                      ...
                      {stakeholder.needs && (
                        <div>
                          <strong>Needs:</strong> {stakeholder.needs}
                        </div>
                      )}
                      {stakeholder.risk && (
                        <div>
                          <strong>Risk:</strong> {stakeholder.risk}
                        </div>
                      )}
                      <div>
                        <strong>Action:</strong> {stakeholder.action}
                      </div>
                    </div>
                  </div>
                ))}
            ...
            <Card>
              <CardHeader>
                <CardTitle className="text-xl font-semibold">This Week's Priorities</CardTitle>
              </CardHeader>
              <CardContent className="p-6 space-y-3">
